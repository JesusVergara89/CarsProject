...
const TestRequestDataComponent = () => {
  ...
  if (status === "loading") {
    ...
    content = posts.map((post, index) => (
      <div key={index} className="w-1/2 h-1/2 sm:h-3/5 bg-white shadow-md rounded-lg p-4">
        ...
        <p className="text-slate-400  text-sm sm:text-base ">{post.description.slice(0, 270)}...</p>
      </div>
    ));
